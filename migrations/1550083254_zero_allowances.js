...
module.exports = function(deployer, networkName) {
  ...
  deployer.then(async function () {
    ...
    async function zeroAllowance (account) {
      console.log(chalk.yellow(`Zeroing allowance for ${account}...`))
      try {
        await zepToken.approve(mockVouching.address, 0, { from: account })
      } catch (error) {
        console.log(chalk.red(`Was unable to zero allowance for ${account}: ${error.message}`))
      }
    }
    ...
    await Promise.all(accounts.concat(extraAccounts).map(account => zeroAllowance(account, MINT_AMOUNT)))
  })
}
